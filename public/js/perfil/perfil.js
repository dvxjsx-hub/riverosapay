...
function abrirCambiarClave() {
  openModal('Cambiar clave', `
    <p class="muted" style="margin:0 0 16px;">Tu clave nueva debe tener entre 8 y 20 caracteres.</p>
    <label>Clave actual<input id="f-clave-actual" type="password" autocomplete="current-password"></label>
    <label>Nueva clave<input id="f-clave-nueva" type="password" minlength="8" maxlength="20" autocomplete="new-password"></label>
    <label>Confirmar nueva clave<input id="f-clave-nueva2" type="password" minlength="8" maxlength="20" autocomplete="new-password"></label>
...
async function guardarCambioClave() {
  ...
  if (!CLAVE_LOGIN_REGEX.test(actual)) { err.textContent = 'Ingresa tu clave actual.'; return; }
  if (!CLAVE_USUARIO_REGEX.test(nueva)) { err.textContent = 'La nueva clave debe tener entre 8 y 20 caracteres.'; return; }
...
async function abrirCodigoRecuperacion() {
  openModal('Código de recuperación', `
    ...
    <label>Clave actual<input id="f-rec-view-pass" type="password" autocomplete="current-password"></label>
...
function pedirConfirmacionEliminarCuenta() {
  ...
  $('#modal-body').innerHTML = `
    ...
    <label>Confirma tu clave<input id="f-del-pass" type="password" autocomplete="current-password"></label>
