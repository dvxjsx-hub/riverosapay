...
// S3: clave normal de 8-20 caracteres para cuentas NUEVAS (registro/recuperación).
const CLAVE_USUARIO_REGEX = /^.{8,20}$/;
const CLAVE_ADMIN_REGEX = /^\d{12}$/;
// Solo para el filtro rápido de /login: acepta clave nueva (8-20) o clave vieja de 6 dígitos,
// para que las cuentas creadas antes de S3 sigan iniciando sesión con su clave de siempre.
const CLAVE_LOGIN_REGEX = /^(?:.{8,20}|\d{6})$/;
...
function configurarCamposClave6() {
  ['#reg-pass', '#reg-pass2', '#f-rec-pass'].forEach(selector => {
    ...
    campo.placeholder = '8-20 caracteres';
  });

  const login = $('#log-pass');
  if (login) {
    login.type = 'password';
    login.removeAttribute('inputmode');
...
function setAuthMode(mode) {
  ...
  $('#auth-toggle').textContent = isLogin ? 'Crear cuenta' : 'Iniciar sesión';
  ...
  if (regUser && regUser.dataset.usernameFilter !== '1') {
    regUser.dataset.usernameFilter = '1';
    regUser.addEventListener('input', () => { regUser.value = (regUser.value || '').replace(/[^a-zA-Z]/g, ''); });
  }
}
...
function setupAuth() {
  ...
  $('#form-register').addEventListener('submit', async (e) => {
    ...
    if (step === 'pass') {
      ...
      if (!CLAVE_USUARIO_REGEX.test(pass)) { err.textContent = 'La clave debe tener entre 8 y 20 caracteres.'; return; }
      setRegisterStep('pass2');
      return;
    }
    const username = $('#reg-user').value.trim().toLowerCase(), pass = $('#reg-pass').value, pass2 = $('#reg-pass2').value;
    if (!CLAVE_USUARIO_REGEX.test(pass2)) { err.textContent = 'La clave debe tener entre 8 y 20 caracteres.'; return; }
  ...
  $('#form-login').addEventListener('submit', async (e) => {
    ...
    if (!CLAVE_LOGIN_REGEX.test(pass) && !CLAVE_ADMIN_REGEX.test(pass)) {
      err.textContent = 'La clave no es válida.';
      return;
    }
    try {
...
function abrirRecuperarContrasena() {
  openModal('Recuperar contraseña', `<label>Usuario<input id="f-rec-user" type="text" autocomplete="username"></label><label>Código de recuperación<input id="f-rec-code" type="text" placeholder="XXXX-XXXX-XXXX"></label><label>Nueva clave<input id="f-rec-pass" type="password" placeholder="8-20 caracteres" minlength="8" maxlength="20" autocomplete="new-password"></label><p class="field-error" id="f-rec-error"></p><button class="btn-primary" onclick="enviarRecuperacion()">Restablecer clave</button>`);
  configurarCamposClave6();
}
...
async function enviarRecuperacion() {
  ...
  if (!CLAVE_USUARIO_REGEX.test(newPassword)) { err.textContent = 'La nueva clave debe tener entre 8 y 20 caracteres.'; return; }
  try {
