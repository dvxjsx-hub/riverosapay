...
(function () {
  function instalar() {
    ...
    window.alternarPermisoAgendaGeneral = async function (jefeId, valor) {
      try {
        await api.patch(`/api/agenda-permiso/${jefeId}`, { puedeVerAgenda: Boolean(valor) });
...
/* Carga el módulo de trabajos personalizados del empleado después de Trabajo. */
(function () {
  if (window.__riverosapayReferenciaLoader) return;
  window.__riverosapayReferenciaLoader = true;
  const script = document.createElement('script');
  script.src = 'js/trabajo/referencia-empleado.js';
  script.async = false;
  document.head.appendChild(script);
})();

/* Carga el módulo de creación de abonos para BOSS. */
(function () {
  if (window.__riverosapayAbonosBossLoader) return;
  window.__riverosapayAbonosBossLoader = true;
  const script = document.createElement('script');
  script.src = 'js/trabajo/abonos-boss.js';
  script.async = false;
  document.head.appendChild(script);
})();
