...
function abrirAccionesTrabajo() {
  const opciones = trabajoVistaActual === TRABAJO_VISTAS.HORARIOS
    ? `<button class="btn-primary" type="button" onclick="openAddTrabajo()">${ICONS.plus} Añadir trabajo</button><button class="btn-secondary" type="button" onclick="abrirFiltroJefe()">Filtrar por BOSS</button><button class="btn-ghost-danger" style="width:100%;margin-top:8px;" type="button" onclick="activarBorradoTrabajo()">Borrar trabajo</button>`
    : `<button class="btn-secondary" type="button" onclick="abrirFiltroPago()">Filtrar pago</button><button class="btn-secondary" type="button" onclick="abrirFiltroJefe(true)">Filtrar por BOSS</button><button class="btn-ghost-danger" style="width:100%;margin-top:8px;" type="button" onclick="activarBorradoTrabajo()">Borrar trabajo</button>`;
...
async function openAddTrabajo() {
  ...
  if (!esJefeAgregando) {
    try {
      ...
      amistadSelectHtml = `<label>BOSS (amistad)<select id="f-trabajo-jefe"><option value="">Sin BOSS · Trabajo personal</option>${opciones}</select></label><p class="field-hint">El BOSS queda fijado al crear el trabajo. Si eliges “Sin BOSS”, será un trabajo personal.</p>`;
...
async function abrirFiltroJefe() {
  try {
    ...
    if (!jefes.length) { openModal('Filtrar por BOSS', '<p class="muted">No hay BOSS registrados en tus trabajos.</p><button class="btn-secondary" onclick="closeModal()">Cerrar</button>'); return; }
    openModal('Selecciona un BOSS', `<button class="btn-secondary" type="button" onclick="aplicarFiltroJefe(null)">Todos los BOSS</button>${jefes.map(j => `<button class="btn-secondary" style="margin-top:8px;" type="button" onclick="aplicarFiltroJefe('${j.id}')">${escapeHtml(j.nombreCompleto || j.username)}</button>`).join('')}`);
...
function openTurnoDetail(turnoId) {
  ...
  if (isJefe) {
    ...
    valorHtml = `<div class="detail-row"><span>BOSS</span><span>${escapeHtml(jefe || 'BOSS asignado')}</span></div><div class="detail-row"><span>Pago</span><span>${valorActual === '' ? 'Sin registrar' : dineroCO(valorActual)} · ${turno.pagado ? 'Pagado' : 'No pagado'}</span></div><div class="notice-box">Solo el BOSS asignado puede añadir o modificar el pago de este trabajo.</div>`;
  }
  ...
  const jefeHtml = isJefe ? `<div class="detail-row"><span>BOSS</span><span>${escapeHtml(jefe || 'Sin BOSS')}</span></div>` : '';
