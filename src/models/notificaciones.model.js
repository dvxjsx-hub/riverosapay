...
function eliminarPorId(usuarioId, notificacionId) {
  const id = String(notificacionId || '').trim();
  if (!id) return null;
  const notif = db.notificaciones.find(n => n.id === id && n.usuarioId === usuarioId);
...
module.exports = { crear, crearParaUsuario, listaCompleta, marcarLeidas, modoDestino, eliminarSolicitudAmistad, eliminarPorId };
