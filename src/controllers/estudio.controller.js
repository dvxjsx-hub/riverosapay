...
async function crearMateria(req, res) {
  ...
  estudio.crearMateria(materia); await save(); estudio.broadcast(empleadoId);
  await auditoria.registrar({ actorId: empleadoId, actorType: 'user', action: 'crear_materia', resource: 'materia', resourceId: materia.id });
  res.json(materia);
}
...
async function actualizarMateria(req, res) {
  ...
  await save(); estudio.broadcast(materia.empleadoId);
  await auditoria.registrar({ actorId: materia.empleadoId, actorType: 'user', action: 'actualizar_materia', resource: 'materia', resourceId: materia.id });
  res.json(materia);
}
...
async function eliminarMateria(req, res) {
  ...
  estudio.eliminarMateria(materia.id); await save(); estudio.broadcast(materia.empleadoId);
  await auditoria.registrar({ actorId: materia.empleadoId, actorType: 'user', action: 'eliminar_materia', resource: 'materia', resourceId: materia.id });
  res.json({ ok: true });
}
...
async function crearActividad(req, res) {
  ...
  estudio.crearActividad(actividad); await save(); estudio.broadcast(empleadoId);
  await auditoria.registrar({ actorId: empleadoId, actorType: 'user', action: 'crear_actividad', resource: 'actividad', resourceId: actividad.id });
  res.json(actividad);
}
...
async function actualizarActividad(req, res) {
  ...
  await save(); estudio.broadcast(actividad.empleadoId);
  await auditoria.registrar({ actorId: actividad.empleadoId, actorType: 'user', action: 'actualizar_actividad', resource: 'actividad', resourceId: actividad.id });
  res.json(actividad);
}
...
async function eliminarActividad(req, res) {
  ...
  estudio.eliminarActividad(actividad.id); await save(); estudio.broadcast(actividad.empleadoId);
  await auditoria.registrar({ actorId: actividad.empleadoId, actorType: 'user', action: 'eliminar_actividad', resource: 'actividad', resourceId: actividad.id });
  res.json({ ok: true });
}
