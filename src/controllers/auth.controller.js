...
const { hashPassword, verifyPassword, newId, newShareCode, newFriendCode, USERNAME_REGEX, PASSWORD_REGEX, LOGIN_PASSWORD_REGEX, hashRecoveryCode, newRecoveryCode } = require('../utils/utils');
...
async function registrar(req, res) {
  ...
  if (!USERNAME_REGEX.test(uname)) { req.authRateLimit?.registrarFallo(); return res.status(400).json({ error: 'El usuario debe tener entre 3 y 15 letras, sin números ni símbolos.' }); }
  if (adminId && uname === adminId) { req.authRateLimit?.registrarFallo(); return res.status(409).json({ error: 'Ese usuario está reservado.' }); }
  if (!PASSWORD_REGEX.test(password || '')) { req.authRateLimit?.registrarFallo(); return res.status(400).json({ error: 'La clave debe tener entre 8 y 20 caracteres.' }); }
  if (usuarios.existeUsername(uname)) { req.authRateLimit?.registrarFallo(); return res.status(409).json({ error: 'Ese usuario ya existe.' }); }
...
async function recuperar(req, res) {
  ...
  if (!user || !user.recoveryCodeHash || hashRecoveryCode((recoveryCode || '').trim()) !== user.recoveryCodeHash) { req.authRateLimit?.registrarFallo(); return res.status(401).json({ error: 'Usuario o código de recuperación incorrectos.' }); }
  if (!PASSWORD_REGEX.test(newPassword || '')) { req.authRateLimit?.registrarFallo(); return res.status(400).json({ error: 'La nueva clave debe tener entre 8 y 20 caracteres.' }); }
...
async function cambiarClave(req, res) {
  ...
  if (!PASSWORD_REGEX.test(nuevaClave || '')) return res.status(400).json({ error: 'La nueva clave debe tener entre 8 y 20 caracteres.' });
...
async function login(req, res) {
  ...
  if (!LOGIN_PASSWORD_REGEX.test(password || '')) { req.authRateLimit?.registrarFallo(); return res.status(401).json({ error: 'Usuario o clave incorrectos.' }); }
