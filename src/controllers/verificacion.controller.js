...
async function enviarSolicitud(req, res) {
  ...
  verificacion.crearSolicitud(solicitud);
  await save();
...
async function marcarLeidas(req, res) {
  notificaciones.marcarLeidas(req.params.empleadoId);
  await save();
  res.json({ ok: true });
}
...
async function responderSolicitud(req, res) {
  ...
  if (solicitud.estado === 'aceptado') {
    ...
    link = verificacion.crearOActualizarLink({
  ...
  if (solicitud.estado === 'aceptado') {
    await notificaciones.crear(solicitud.empleadoId, 'jefe_configurado', { jefeUsername: solicitud.jefeUsername });
  } else {
    getIO().to('emp-' + solicitud.empleadoId).emit('notificaciones:update');
  }
...
async function datosEmpleado(req, res) {
  ...
  res.json({
    ...
    ...trabajo.snapshot(req.params.empleadoId)
  });
}
...
async function estudioEmpleado(req, res) {
  ...
  if (!verificacion.tieneAcceso(jefeId, req.params.empleadoId)) return res.status(403).json({ error: 'No tienes acceso verificado a este empleado.' });
  res.json({
    materias: estudio.materiasDe(req.params.empleadoId),
    actividades: estudio.actividadesDe(req.params.empleadoId)
  });
}
...
module.exports = {
  obtenerCodigo, enviarSolicitud,
  listarNotificaciones, marcarLeidas, solicitudesPendientes, responderSolicitud,
  historial, datosEmpleado, estudioEmpleado, eventoEmpleado
};
