...
function horarioSeSolapa(inicioA, finA, inicioB, finB) {
  return String(inicioA || '') < String(finB || '') && String(finA || '') > String(inicioB || '');
}

function diaSemanaDeFecha(fecha) {
  return new Intl.DateTimeFormat('es-CO', { weekday: 'long' }).format(new Date(`${fecha}T12:00:00`));
}

function diaNormalizado(dia) {
  return String(dia || '').trim().toLowerCase();
}

function conflictoAgendaEmpleado(empleadoId, fecha, horaInicio, horaFin) {
  const diaSemana = diaNormalizado(diaSemanaDeFecha(fecha));

  const trabajoExistente = (db.turnos || []).find(t =>
    t.empleadoId === empleadoId &&
    t.fecha === fecha &&
    horarioSeSolapa(horaInicio, horaFin, t.horaInicio, t.horaFin)
  );
  if (trabajoExistente) {
    return { tipo: 'trabajo', mensaje: 'No puedes agendar este trabajo porque el empleado ya tiene otro trabajo en ese horario para esa fecha.' };
  }

  const materiaExistente = (estudio.materiasDe(empleadoId) || []).find(m =>
    diaNormalizado(m.dia) === diaSemana &&
    horarioSeSolapa(horaInicio, horaFin, m.horaInicio, m.horaFin)
  );
  if (materiaExistente) {
    return { tipo: 'academico', mensaje: `No puedes agendar este trabajo porque el empleado tiene clase de ${materiaExistente.nombre} en ese horario.` };
  }
...
async function crearTurno(req, res) {
  ...
  if (!/^\d{2}:\d{2}$/.test(horaInicio) || !/^\d{2}:\d{2}$/.test(horaFin) || horaInicio >= horaFin) return res.status(400).json({ error: 'El horario del trabajo no es válido.' });
  ...
  if (creandoComoBoss) {
    const conflicto = conflictoAgendaEmpleado(empleadoId, fecha, horaInicio, horaFin);
    if (conflicto) return res.status(409).json({ error: conflicto.mensaje });
  }
  ...
  await save();
  await auditoria.registrar({ actorId, actorType: 'user', action: 'crear_trabajo', resource: 'trabajo', resourceId: turno.id });
...
async function finalizarTurno(req, res) {
  ...
  await save();
  await auditoria.registrar({ actorId, actorType: 'user', action: 'finalizar_trabajo', resource: 'trabajo', resourceId: turno.id });
  trabajo.broadcast(turno.empleadoId);
...
module.exports = { obtenerSnapshot, obtenerMisJefes, obtenerTrabajosComoJefe, crearTurno, crearSolicitudTrabajo, responderSolicitudTrabajo, actualizarTurno, actualizarPermisoAgenda, finalizarTurno };