...
async function crearTurno(req, res) {
  ...
  if (asignado && asignado !== empleadoId && !amistades.existe(empleadoId, asignado)) return res.status(400).json({ error: 'El jefe seleccionado debe ser una de tus amistades.' });
  ...
  if (actorJefeId && actorJefeId !== empleadoId) await notificaciones.crearParaUsuario(empleadoId, 'trabajo_añadido', { jefeUsername: actorJefeUsername || 'Tu jefe', lugar: lug.nombre });
...
async function actualizarTurno(req, res) {
  ...
  if (req.body.jefeAsignadoId !== undefined) {
    ...
    if (nuevoJefe && nuevoJefe !== turno.empleadoId && !amistades.existe(turno.empleadoId, nuevoJefe)) return res.status(400).json({ error: 'El jefe seleccionado debe ser una de las amistades del usuario.' });
...
async function eliminarTurno(req, res) {
  ...
  turno.eliminacionPendiente = true; await save(); trabajo.broadcast(turno.empleadoId); res.json({ eliminado: false, pendiente: true });
}
...
async function confirmarEliminacion(req, res) {
  ...
  await notificaciones.crearParaUsuario(turno.empleadoId, 'trabajo_eliminado', { jefeUsername: req.body.jefeUsername || 'Tu jefe', lugar: lug ? lug.nombre : '' }); res.json({ eliminado: true });
}
...
async function rechazarEliminacion(req, res) {
  ...
  const { jefeId } = req.body || {};
  if (!verificacion.tieneAcceso(jefeId, turno.empleadoId) && !trabajo.tieneTrabajoAsignado(jefeId, turno.empleadoId)) return res.status(403).json({ error: 'No tienes acceso a este trabajo.' });
  turno.eliminacionPendiente = false; await save(); trabajo.broadcast(turno.empleadoId); await notificaciones.crearParaUsuario(turno.empleadoId, 'trabajo_eliminacion_rechazada', { jefeUsername: req.body.jefeUsername || 'Tu jefe' }); res.json({ eliminado: false });
}
