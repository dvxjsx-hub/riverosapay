...
const EMPTY_DB = {
  ...
  trabajoSolicitudes: [], trabajadoresPersonal: [], tesorerias: [], tesoreriaSolicitudes: [], tesoreriaMovimientos: [], sessions: [], abonos: []
};
...
const COLECCIONES_INDEPENDIENTES = {
  ...
  abonos: ['abonos', ['jefeId', 'empleadoId', 'fecha']]
};

const mongoCollections = {};
// S/F2: guarda la última firma (JSON) de cada colección ya sincronizada con Mongo.
// Si una colección no cambió desde el save() anterior, guardarColeccion() no hace
// ningún viaje a la base — antes, cada save() reescribía las 16 colecciones
// completas sin importar cuál había cambiado realmente.
const ultimaFirma = {};
...
async function guardarColeccion(clave, collection) {
  ...
  const actuales = Array.isArray(db[clave]) ? db[clave] : [];
  const firma = JSON.stringify(actuales);
  if (ultimaFirma[clave] === firma) return; // nada cambió: no hacemos ningún viaje a Mongo
...
async function init() {
  ...
  for (const [clave, [nombre, indices]] of Object.entries(COLECCIONES_INDEPENDIENTES)) {
    ...
    ultimaFirma[clave] = JSON.stringify(db[clave]);
  }
